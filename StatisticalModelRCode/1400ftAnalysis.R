...
library(dplyr)
# Read Data (1,400 ft.)
crash <- read.csv("Crashes1400ftBusStopNICC.csv", na = c("#N/A", "NA"))
...
crash2 <- crash %>%
  left_join(
    bus_ref %>% 
      filter(!is.na(FID)) %>%                # Removes the 51 NAs
      distinct(FID, .keep_all = TRUE) %>%    # Ensures every FID is unique
      mutate(stopabbr = as.character(stopabbr)),
    by = c("NEAR_FID" = "FID")
  )

bus_int_single <- bus_int %>%
  filter(uta_stopid != 0 & !is.na(uta_stopid)) %>% # Removes ID 0 and any NAs
  group_by(uta_stopid) %>%
...
#Convert categories to numeric for glm fit
...
#summary(crash3)
newdat<-data.frame(SPEED_LMT = 35, DOT_AADT = 20000, AvgBoard = 5, 
                   AvgAlighting = 5, Intersecti = "Y", 
                   Bus_Stop_Location = "FS", Bus_Stop_Land_Use = "Commercial",
                   StopIntDist = 158.663, CrashStopDist = seq(0.58, 1487.834,length.out = 100))
...
plot(pred1~CrashStopDist,data = newdat, type = "l", xlab = "Distance of a Crash from Bus Stop", ylab = "Probability (%) a Crash is VRU Related",main = "VRU Crash Probability 1,400 ft.")
#Use the median values of each predictor category to hold them all constant

newdata<-data.frame(CrashStopDist = 286.8, DOT_AADT = 20000, AvgBoard = 5, 
                   AvgAlighting = 5, Intersecti = "Y", 
                   Bus_Stop_Location = "FS", Bus_Stop_Land_Use = "Commercial",
                   StopIntDist = 158.663, SPEED_LMT = seq(0, 65, length.out = 100))
#Use the median values of each predictor category to hold them all constant

newdata$pred1 <- predict(model4,newdata,type = "response")
plot(pred1~SPEED_LMT,data = newdata, type = "l", xlab = "Speed Limit (mph)", ylab = "Probability (%) a Crash is VRU Related",main = "VRU Crash Probability 1,400 ft.")
#Use the median values of each predictor category to hold them all constant

library(emmeans)
em4.1 <- emmeans(model4.1, ~Bus_Stop_Land_Use)
plot(em4.1, type="response", xlab = "Probability (%) a Crash is VRU Related (1,400 ft.)", ylab = "Bus Stop Land Use", main = "VRU Crash Probability 1,400 ft.")
plot(pairs(em4.1, type="response"))
em4.2 <- emmeans(model4.1, ~Bus_Stop_Location)
plot(em4.2, type="response", xlab = "Probability (%) a Crash is VRU Related (1,400 ft.)", ylab = "Bus Stop Location", main = "VRU Crash Probability 1,400 ft.")
